...
def _score_single_criterion(client, model: str, application_text: str, criterion: dict, agency: str, nofo_text: str, page_count: int, budget_rules: dict | None = None) -> dict[str, Any]:
    ...
    requirement_assessment = {
        ...
        "properties": {
            ...
            "explanation": {"type": "string", "description": "1-3 sentence reviewer comment. CRITICAL: If this criterion has subcriteria, the explanation MUST begin with the subcriterion name in square brackets, e.g. '[Approach] The applicant organization...' or '[High-level work plan] The applicant organization...'. This prefix is required for the UI to group assessments correctly."},
    ...
    if subcriteria_defs:
        ...
        sub_instruction = f"\n\nSUBCRITERIA (score each individually — scores must sum to the parent criterion total):\n{sub_text}\n\nYou MUST return a subcriteria array with exact names and point allocations matching the list above. Each subcriterion score must be between 0 and its maximum. The sum of subcriterion scores must equal the parent criterion score.\n\nABSOLUTE RULE — SUBCRITERION PREFIX ON EVERY EXPLANATION:\nEvery requirement_assessment explanation MUST begin with the subcriterion name in square brackets. This is NOT optional. The UI uses these prefixes to group assessments under their correct subcriterion heading.\n\nFormat: '[Subcriterion Name] The applicant organization...'\nExamples for this criterion:\n" + "\n".join(f"  '[{s['name']}] The applicant organization...'" for s in subcriteria_defs) + "\n\nIf you omit the bracket prefix, the assessment will be ungrouped and the reviewer cannot see which subcriterion it belongs to. EVERY explanation must start with one of the bracket prefixes listed above.\n\nGroup strengths, mets, and weaknesses by subcriterion as well."
